...
public class InvoiceController implements Initializable {
    ...
    public ObservableList<salesModel> loadData() throws SQLException {
        ...
        while(resultSet.next()){
            ...
            Date sales_date = resultSet.getDate("sales_date");
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        bt_generate.setOnAction(event -> {
            ...
            if(tf_id.getText().isEmpty()){
                ...
                String path = null;
                if (token == 1) {
                    path = "E:/Invoice " + tf_cname.getText() + " " + dtf.format(now) + ".pdf";
                    file = new File("E:/Invoice " + tf_cname.getText() + " " + dtf.format(now) + ".pdf");
                } else {
                    path = "E:/Invoice " + dtf.format(now) + ".pdf";
                    file = new File("E:/Invoice " + dtf.format(now) + ".pdf");
                }
                ...
                try {
                    ...
                    while (resultSet.next()) {
                        ...
                        sales_date = resultSet.getDate("sales_date");
                ...
                try {
                    con = DriverManager.getConnection("jdbc:postgresql://localhost:5432/HUNGRIMA", "postgres", "1234");
                    psD = con.prepareStatement("SELECT * FROM orders WHERE s_id = ?");
                    psD.setInt(1, s_id);
                    rs = psD.executeQuery();
                    while (rs.next()) {
