...
public class SalesController implements Initializable {
    ...
    public void deduct(Integer quantity, String name) throws SQLException {

        try {
            connection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/HUNGRIMA", "postgres", "1234");
            psD = connection.prepareStatement("SELECT quantity FROM products WHERE pname = ?");
        ...
        bt_msale.setOnAction(event -> {
            if(tf_cname.getText().isEmpty()||tf_total.getText().isEmpty()||mpcombo.getText().isEmpty()||tf_sdate.getText().isEmpty()){
                ...
                try {
                    ...
                    psInsert.setDate(6, Date.valueOf(tf_sdate.getValue()));
                    ...
                    for (int i = 0; i < bill.size(); i++){
                        ...
                        deduct(Integer.valueOf(bill.get(i).getQuantity()), bill.get(i).getPname());

                        DBUtils.track(event, bill.get(i).getPname(),Date.valueOf(tf_sdate.getValue()), "sale", Integer.parseInt(bill.get(i).getQuantity()), newQ, Math.toIntExact(id));
